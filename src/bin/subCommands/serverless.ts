...
import { IAwsCredentials } from "../../getAwsCredentials";
...
const SLS_CONFIG_DIRECTORY = path.join(process.cwd(), "serverless-config");
...
export function detectIfServerlessMicroserviceProject() {
  return fs.existsSync(path.join(SLS_CONFIG_DIRECTORY, "provider.ts"));
}
...
/** tests whether the running function is running within Lambda */
